        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="card-img card-img__max" style="background-image: url({{{$post->img ?? asset('img/img.jpeg')
}}}"></div>

                    <div class="card-descr">Описание: {{$post->text}}</div>
                    <div class="card-user">Автор: {{$post->name}}</div>
                    ...
                    <div class="card-btn">
                        ...
                        <form action="{{route('post.destroy', ['id'=>$post->post_id])}}" method="post"
                              onsubmit="if(confirm('Удалить запись?')) {return true} else {return false} ">
                            @csrf
                            @method ('DELETE')
